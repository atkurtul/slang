use crate::ast;
pub use crate::ast::{Aggregate, Binding, Binop, Literal, Pattern, Prim, Sstr, Unop};
use crate::memory::*;
use crate::*;
...
impl Ty {
    ...
    pub fn agg(v: &Vec<ast::Ty>, ctx: &Context) -> Rc<[Ty]> {
    ...
    pub fn new(ty: &ast::Ty, ctx: &Context) -> Ty {
        match ty {
            ...
            ast::Ty::Adt(s) => Ty::Adt(ctx.find_named_type(s).unwrap()),
            ast::Ty::Gen(x, y) => Ty::Gen(*x, *y),
            ast::Ty::Gadt(n, g) => Ty::Gadt(ctx.find_named_type(n).unwrap(), Ty::agg(g, ctx)),
            ast::Ty::Func(args, ret) => Ty::Func(Ty::agg(args, ctx), Ty::boxed(ret, ctx)),
        }
    }
    ...
    pub fn solved(&self) -> bool {
        match self {
            ...
            Ty::Func(args, ret) => args.iter().fold(true, |x, y| x && y.solved()) && ret.solved(),
            Ty::Var(_) => false,
        }
    }
}
...
pub enum Expr {
    ...
    Call(Rc<Node>, Vec<Ty>, Vec<Rc<Node>>),
    Aggregate(Aggregate<Rc<Node>>),
...
impl Func {
    pub fn get_args(&self) -> Rc<[Ty]> {
        if let Ty::Func(args, _) = &self.ty {
            return args.clone();
        }
        unreachable!()
    }
    pub fn get_ret(&self) -> &Ty {
        if let Ty::Func(_, ret) = &self.ty {
            return &ret;
        }
        unreachable!()
    }
...
impl Context {
    ...
    fn check(&self) -> bool {
        self.nodes.iter().fold(true, |x, y| {
            x && match &y.x {
                Expr::Ctx(ctx) => ctx.check(),
                _ => y.t.solved(),
            }
        })
    }
        ...
        for x in x.iter() {
            match x {
                ...
                ast::Expr::Func {
                    ...
                    ctx.collect(&body);
                    let ctx = Rc::new(ctx);
                    //let ctx = self.new_node(Some(ctx.represents.ty), Expr::Ctx(ctx));
                    if this.is_some() {
                ...
                ast::Expr::Type {
                    ...
                    let st = Rc::new(Adt {
                        name,
                        types: mems.into_iter().map(|t| Ty::new(t, self)).collect(),
    ...
    pub fn lower(&mut self, x: &ast::Expr) -> Rc<Node> {
        match x {
            ...
            ast::Expr::Binary(op, l, r) => {
                ...
                let r = self.lower(r);
                l.unify_ty(&r.t);
                let x = self.new_node(None, Expr::Binary(*op, l.clone(), r));
                match op {
                    Binop::Lt
                    | Binop::Le
                    | Binop::Ge
                    ...
                    | Binop::Eq
                    | Binop::Neq
                    | Binop::And => x.unify_ty(&Ty::Prim(Prim::Bool)),
                    _ => x.unify_ty(&l.t),
                }
                x
            }
            ...
            ast::Expr::Cast(t, x) => {
                let t = Ty::new(&t, self);
                let l = self.lower(x);
                let x = self.new_node(Some(t.clone()), Expr::Cast(t, l.clone()));
                x
            }
            ...
            ast::Expr::Call(l, _, r) => {
                ...
                match &l.t {
                    Ty::Func(args, ret) => {
                        ...
                        self.new_node(Some(ret.as_ref().clone()), Expr::Call(l, vec![], r))
                    }
                    what => unreachable!(),
                }
            }
            ...
            ast::Expr::FreeCall(l, _, r) => {
                ...
                match &f.t {
                    Ty::Func(args, ret) => {
                        ...
                        self.new_node(Some(ret.as_ref().clone()), Expr::Call(f, vec![], r))
                    }
                    _ => unreachable!(),
                }
            }
...
impl Node {
    pub fn unify_ty(&self, t: &Ty) {
        ...
        match (&self.x, t.clone()) {
            ...
            (Expr::This | Expr::Nil | Expr::Arg | Expr::Va, t) => {
